...
Algebra_Node* DerivateFunc(Algebra_Node* root , Debug_concept W) // Следить , чтобы у каждой ветви было возвращающие значение.
{
	Token& T = root->data;
	...
	else if (f_opr_two(T))
	{
		...
		P[0] = root->left;
		P[1] = root->right;
		if (DEBUG) W.call_order ^= 1; // выполняем инвертирование.
